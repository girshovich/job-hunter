...
async function runBatch(
  ...
  for (const { company, logoUrl } of results) {
    upsert.run(company, logoUrl, now);
...
async function main() {
  ...
  const rows = db.prepare(`
    SELECT DISTINCT j.company, j.job_source
    FROM jobs j
    LEFT JOIN companies c ON c.company = j.company
    WHERE c.logo_url IS NULL
    ORDER BY j.company
  `).all() as Array<{ company: string; job_source: string }>;
  ...
  const work = rows.map(({ company, job_source }) => {
    const slug = job_source === 'Ashby' ? slugMap.get(company) : undefined;
    const domain = slug ? `${slug}.com` : deriveCompanyDomain(company);
